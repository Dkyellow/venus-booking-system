...
with app.app_context():
    ...
    # --- Sync service categories (always run to ensure existing DB has categories) ---
    ...
    # --- Sync services WITH category_id (always ensure category_id is assigned) ---
    service_data = [
    ...
    # Deactivate removed services (e.g. Eye Services, ENT)
    ...
    for svc in Service.query.filter_by(is_active=True).all():
        ...
        if ln in retired_exact or any(term in ln for term in retired_terms):
            svc.is_active = False
            print(f"[MIGRATE] Deactivated retired service: {svc.name}")
    db.session.flush()
    ...
    for first, last, email, spec, title, color, svc_names in practitioners_data:
        ...
        if not s:
            ...
            db.session.flush()
        for sn in svc_names:
            if sn in services and services[sn] not in s.services:
                s.services.append(services[sn])
        for day in range(7):
            existing_sched = StaffSchedule.query.filter_by(staff_id=s.id, day_of_week=day).first()
    ...
    db.session.commit()
    print(f"[SYNC] Database sync complete: {len(categories)} categories, {len(services)} active services.")
